...
int Dosya::ara(char aranacak[]){
	FILE *yenidosya; // Kayýt sýralarýný tutmak için açýlacak dosya için pointer
    Tel_Kayit k;
	int sayac=0;
	bool tumu=false;
	int bulunan=0;
	...
	yenidosya = fopen("kayitno.txt", "w+"); // Kayýt sýralarýný tutmak için dosya açýlýyor
    while(!feof(teldefteri)){		
        ...
        fprintf(yenidosya ,"%d ",sayac); // Kayýt bulundukça dosyaya yazýlýyor
        bulunan++;
	}	
	fclose(yenidosya);// Kayýt arama iþlemi bitince dosya kapatýlýyor
    return bulunan;
}
